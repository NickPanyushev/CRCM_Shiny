...
#fwrite(input_table, "~/Job_search/GeneStack/input_file.tsv", sep = "\t")
input_table <- fread("~/Job_search/GeneStack/input_file.tsv")
...
opts <- structure(input_table$go_term_label, names = input_table$gene_symbol) #- this string works, not good, but it does!
opts <- unique(c(input_table$go_term_label, input_table$gene_symbol))
opts[is.na(opts)] <- ""
...
server <-  function(input, output) {
  ...
  data <- reactive({
    input_table[go_term_label %in% input$geneinput, .(gene_symbol, gene_synonyms, go_term_label)]
  })
...
opts_example <- sapply(1:100000, function(i) paste0(sample(letters, 9), collapse=""))
